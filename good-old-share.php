<?php
/*
Plugin Name: Good old Share
Plugin URI: http://www.whiletrue.it/it/really-simple-share-wordpress-plugin/
...
require_once('good-old-share-options.php');
require_once('good-old-share-counts.php');


// RETRIEVE OPTIONS

$good_old_share_option = good_old_share_get_options_stored();


// ACTION AND FILTERS

add_action('init', 'good_old_share_init');
add_action('admin_menu', 'good_old_share_menu');
add_filter('plugin_action_links', 'good_old_share_add_settings_link', 10, 2);
add_shortcode( 'good_old_share', 'good_old_share_shortcode' );

if ($good_old_share_option['scripts_at_bottom']) {
	add_action('wp_footer', 'good_old_share_scripts');
} else {
	add_action('wp_head',   'good_old_share_scripts');
}

if (($good_old_share_option['active_buttons']['facebook_like'] && $good_old_share_option['facebook_like_html5'])
  || $good_old_share_option['active_buttons']['facebook_share_new']) {
	add_action('wp_footer', 'good_old_share_facebook_like_html5_bottom_scripts');
}

if (!$good_old_share_option['disable_default_styles']) {
	add_action('wp_print_styles', 'good_old_share_style');
}

add_filter('the_content', 'good_old_share_content');
if (!$good_old_share_option['disable_excerpts']) {
	add_filter('the_excerpt', 'good_old_share_excerpt');
}


// PUBLIC FUNCTIONS

function good_old_share_scripts () {
  // IF PERFORMANCE MODE IS ACTIVE, CHECK SKIP BUTTONS
  if (good_old_share_skip_buttons(true)) {
		return '';
  }

	good_old_share_adjust_locale();
	
	global $good_old_share_option;

	$out = '';

	if ($good_old_share_option['active_buttons']['twitter']) {
		$out .= '
      !function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="https://platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");
      ';
  }

	if ($good_old_share_option['active_buttons']['google1'] 
	||  $good_old_share_option['active_buttons']['google_share']
	||  $good_old_share_option['active_buttons']['youtube']) {
		$out .= '
      window.___gcfg = {lang: "'.substr($good_old_share_option['locale'],0,2).'"};
		  (function() {
	...
	if ($good_old_share_option['active_buttons']['pinterest']) {
		$hover = ($good_old_share_option['pinterest_hover']!='') ? ' p.setAttribute(\'data-pin-hover\', true); ' : '';
		$out .= '
			(function(d){
...
function good_old_share_facebook_like_html5_bottom_scripts () {
  // IF PERFORMANCE MODE IS ACTIVE, CHECK SKIP BUTTONS
  if (good_old_share_skip_buttons(true)) {
		return '';
  }
	...
	$app_id = (is_numeric($good_old_share_option['facebook_like_appid'])) ? '&appId='.$good_old_share_option['facebook_like_appid'] : '';
	$out = '
  <div id="fb-root"></div>
  <script>(function(d, s, id) {
    ...
    js.src = "//connect.facebook.net/'.$good_old_share_option['locale'].'/sdk.js#xfbml=1'.$app_id.'&version=v2.0";
...
function good_old_share_init ($force=false) {
	load_plugin_textdomain('good-old-share', false, basename(dirname(__FILE__)).'/lang');
  ...
  if (good_old_share_skip_buttons(true)) {
	...
	if ($good_old_share_option['active_buttons']['linkedin']) {
		wp_enqueue_script('good_old_share_linkedin', 'https://platform.linkedin.com/in.js', array(), false, $good_old_share_option['scripts_at_bottom']);
	}
	// BUFFER JS ONLY WORKS ON BOTTOM
	if ($good_old_share_option['active_buttons']['buffer'] and $good_old_share_option['scripts_at_bottom']) {
		wp_enqueue_script('good_old_share_buffer', 'http://static.bufferapp.com/js/button.js', array(), false, $good_old_share_option['scripts_at_bottom']);
	}
	if ($good_old_share_option['active_buttons']['flattr']) {
		wp_enqueue_script('good_old_share_flattr', 'https://api.flattr.com/js/0.6/load.js?mode=auto&#038;ver=0.6', array(), false, $good_old_share_option['scripts_at_bottom']);
	}
  // FRYPE LIB HAS TO BE IN THE HEADER
	if ($good_old_share_option['active_buttons']['frype']) {
		wp_enqueue_script('good_old_share_frype', 'https://www.draugiem.lv/api/api.js', array(), false);
	}
	if ($good_old_share_option['active_buttons']['tumblr']) {
		wp_enqueue_script('good_old_share_tumblr', 'http://platform.tumblr.com/v1/share.js', array(), false, $good_old_share_option['scripts_at_bottom']);
	}
	if ($good_old_share_option['active_buttons']['bitcoin'] || $good_old_share_option['active_buttons']['litecoin']) {
    // ALWAYS IN THE HEADER, OTHERWHISE THE WIDGET IS UNABLE TO LOAD
		wp_enqueue_script('good_old_share_bitcoin', 'http://coinwidget.com/widget/coin.js');
	}
}    


function good_old_share_style () {
  // IF PERFORMANCE MODE IS ACTIVE, CHECK SKIP BUTTONS
  if (good_old_share_skip_buttons(true)) {
		return '';
  }
	...
	if ( file_exists($myStyleFile) ) {
	    wp_register_style('good_old_share_style', $myStyleUrl);
	    wp_enqueue_style ('good_old_share_style');
	}
}


function good_old_share_menu () {
	add_options_page('Good old share Options', 'Good old share',  'manage_options', 'good_old_share_options', 'good_old_share_options');
	add_options_page('Good old share Counts',  'Good old counts', 'manage_options', 'good_old_share_counts',  'good_old_share_counts');
}


function good_old_share_add_settings_link($links, $file) {
	static $this_plugin;
	if (!$this_plugin) $this_plugin = plugin_basename(__FILE__);
 
	if ($file == $this_plugin){
		$settings_link = '<a href="admin.php?page=good_old_share_options">'.__("Settings").'</a>';
...
function good_old_share_content ($content) {
	return good_old_share ($content, 'the_content');
}


function good_old_share_excerpt ($content) {
	return good_old_share ($content, 'the_excerpt');
}


// FUNCTION USED TO CHECK IF I NEED TO SKIP THE SHARE BUTTONS IN THE GIVEN CONTEXT
function good_old_share_skip_buttons ($check_performance_mode = false) {
	global $good_old_share_option;
	$option = $good_old_share_option;
...
function good_old_share ($content, $filter, $link='', $title='', $author='', $force_button='') {
	...
	if ($filter=='the_excerpt' and $last_execution=='the_content') {
		...
		remove_filter('the_content', 'good_old_share_content');
	...
	$custom_field_disable = get_post_custom_values('good_old_share_disable');
	...
	if ($filter!='shortcode') {
    if (good_old_share_skip_buttons()) {
			return $content;
    }
    /*
    // OLD STYLE CHECK
    ...
    */
	...
	$div_button_open = '<div style="min-height:'.$height.'px;" class="good_old_share good_old_share_'.$option['layout'].' robots-nocontent snap_nopreview">';
	...
	foreach (explode(',',$option['sort']) as $name) {
		...
		// OPEN THE BUTTON DIV
		$out .= '<div class="good_old_share_'.$name.'" style="width:'.$option['width_buttons'][$name].'px;">';
			...
			if ($option['facebook_like_html5']) {
  			...
  			$option_height = ($option['layout']=='box') ? 62 : 27;
				...
				if ($option['facebook_like_send']) {
					...
					$out .= '
						<div class="good_old_share_facebook_like_send">
      ...
      $facebook_link = ($option['facebook_like_fixed_url']!='') ? $option['facebook_like_fixed_url'] : $link;
			...
			$out .= '<a href="mailto:?subject='.rawurlencode($subject).'&amp;body='.rawurlencode($subject.' - '.$link).'"><img src="'.plugins_url('images/email.png',__FILE__).'" alt="'.__('Email', 'good-old-share').'" title="'.__('Email', 'good-old-share').'" /> '.stripslashes($option['email_label']).'</a>';
		}
		else if ($name == 'print') {
			$out .= '<a href="javascript:window.print();void(0);"><img src="'.plugins_url('images/print.png',__FILE__).'" alt="'.__('Print', 'good-old-share').'" title="'.__('Print', 'good-old-share').'" /> '.stripslashes($option['print_label']).'</a>';
		}
      ...
      $out .= '<a href="javascript:void(0);" onclick="window.open(\''.$option['specificfeeds_link'].'\',\'EmailSubscribe\',\'toolbar=yes,location=no,status=no,menubar=no,scrollbars=no,resizable=no,width=400,height=400,left=430,top=23\'); return false;">'
          .'<span class="super">'.__('Subscribe', 'good-old-share').':</span> <img src="http://www.specificfeeds.com/theme/classic/img/sf_20.png" alt="SpecificFeeds" title="SpecificFeeds" /></a>';
	...
	if ($option['above_prepend_above']!='') {
		$above_prepend_above = '<div class="good_old_share_prepend_above robots-nocontent snap_nopreview">'.stripslashes($option['above_prepend_above']).'</div>';
	}
	// BELOW PREPEND ABOVE TEXT
	$below_prepend_above = '';
	if ($option['below_prepend_above']!='') {
		$below_prepend_above = '<div class="good_old_share_prepend_above robots-nocontent snap_nopreview">'.stripslashes($option['below_prepend_above']).'</div>';
	}
	// ABOVE PREPEND INLINE TEXT
	$above_prepend_inline = '';
	if ($option['above_prepend_inline']!='') {
		$above_prepend_inline = '<div class="good_old_share_prepend_inline">'.stripslashes($option['above_prepend_inline']).'</div>';
	}
	// BELOW PREPEND INLINE TEXT
	$below_prepend_inline = '';
	if ($option['below_prepend_inline']!='') {
		$below_prepend_inline = '<div class="good_old_share_prepend_inline">'.stripslashes($option['below_prepend_inline']).'</div>';
	}
...
function good_old_share_adjust_locale () {
	if (defined("ICL_LANGUAGE_CODE") and ICL_LANGUAGE_CODE!='') {
		global $good_old_share_option, $wpdb, $table_prefix;
		...
		$good_old_share_option['locale'] = ($full_locale!='') ? $full_locale : ICL_LANGUAGE_CODE;
	}
}


// SHORTCODE FOR ALL ACTIVE BUTTONS
function good_old_share_shortcode ($atts) {
	extract( shortcode_atts( array(
		'button' => '',
	), $atts ) );
	
	return good_old_share ('', 'shortcode', '', '', '', $button);
}


//FUNCTION AVAILABLE FOR EXTERNAL INCLUSION, INSIDE THEMES AND OTHER PLUGINS
function good_old_share_publish ($link='', $title='') {
	return good_old_share ('', 'shortcode', $link, $title);
}
...
function good_old_share_box_content ($title, $content) {
	if (is_array($content)) {
		$content_string = '<table>';
		foreach ($content as $name=>$value) {
			$content_string .= '<tr>
				<td style="width:130px;">'.__($name, 'good-old-share' ).':</td>	
				<td>'.$value.'</td>
				</tr>';
		}
		$content_string .= '</table>';
...
function good_old_share_get_options_stored () {
	//GET ARRAY OF STORED VALUES
	$option = get_option('good_old_share');
  if (!is_array($option)) {
    $option = array();
  } 
	...
	$option_default = good_old_share_get_options_default();
...
function good_old_share_get_options_default () {
	$option = array();
